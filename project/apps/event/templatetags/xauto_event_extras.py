...
@register.inclusion_tag('partials/categories_list.html')
def categories_list(category, active_category):
    return {
        'categories': get_categories_with_parent(category),
        'active': active_category,
    }

@register.inclusion_tag('partials/side_categories_list.html')
def parent_categories_list(category, service_id = 0, selected_categories = None):
    return {
        'categories': get_categories_with_child(category, selected_categories),
        'active': int(category),
    }
...
category_breadcrumbs_cache = {}
@register.inclusion_tag('partials/category_breadcrumbs.html')
def category_breadcrumbs(category_id, service_id=None, link_service=False, captions = True):
    SEPARATOR = u'&nbsp;&#47;&nbsp;'
    global category_breadcrumbs_cache
    if len(category_breadcrumbs_cache) == 0:
        categories_breadcrumbs = {}
    breadcrumbs = []
    key = "%s:%s:%s:%s" % (category_id, service_id, link_service, captions)
    value = category_breadcrumbs_cache.get(key, False)
    if value:
    ...
    if service_id:
        ...
        breadcrumbs[-1]['text'] = s.replace(SEPARATOR, '')
        servicetype = ServiceType.objects.get(id=service_id)
        breadcrumbs.append({'url': reverse('event_list', args=[service_id]) if service_id else '',
                            'text': ': ' + servicetype.name,
                            'link': link_service,
                           # 'servicetype_object': servicetype
                            })
    else:
        if len(breadcrumbs) > 0:
            s = breadcrumbs[-1]['text']
            breadcrumbs[-1]['text'] = s.replace(SEPARATOR, '')
    category_breadcrumbs_cache[key] = breadcrumbs
    return {
