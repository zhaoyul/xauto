...
#===================================================
#===================================================
# === C H E C K   I S O      D A T E             ===
# === Return 9 TUPLE for Further Date processing ===
# === wmode  => CHECK ONLY                       ===
# === wnow   => Forbidd any date in the past     ===
#===================================================
#===================================================

def CheckIsoDate(mysqlTimeStampStr, wmode = '', wnow = 'NOW', wtime = '', default=None):
  "given a mysql timestamp string, return a 9-tuple for use with time module"

  #=================================================
  # sample mysql timestamp: '2001-09-18 23:41:53'
  # Tuple sould have at least 5 Tuples            
  result = default                                           # default return None Value (error) ==
  if wtime == '':
    DateFormat = '%Y-%m-%d %H:%M:%S'                         #== ISO date Format  + Time         ==
  ...
  #==================================================================
  #=== check if Current date is set in the past (ex : Birth Date) ===
  minYear = 2001
  if wnow == 'PAST':
    minYear = 1900

  #============================================
  #===== Check basic Controle on ISO date =====
  try:
    wdatetuple =  time.strptime(mysqlTimeStampStr,DateFormat)    # == extract each Date/Time field  ISO Format ==
  except:
    return result

  #====================================================
  #==== Check Logical on Date and build DateTuple ===== 
  try:
    t0 = int(wdatetuple[0])       # extract and check year   (between 2001 and 2037)
    if (t0 < minYear or t0 > 2037):
      return result
    t1 = int(wdatetuple[1])       # extract and check month  (1 to 12)
    if (t1 < 0 or t1 > 12):
      return result
    t2 = int(wdatetuple[2])       # extract and check day   (1 to 28,29,30 or 31 depending selected Month)
    if (t2 < 0 or t2 > MaxDayMonth(string.zfill(t1,2),t0)):
      return result
    #======================================================
    #====== Check Time (by default / Iso Date format) ======
    if wtime == '':
      t3 = int(wdatetuple[3])       # extract and check hour  (00 to 23)
      if (t3 < 0 or t3 > 23):
        return result
      t4 = int(wdatetuple[4])       # minute (00 to 59)
      if (t4 < 0 or t4 > 59):
        return result
      t5 = int(wdatetuple[5])       # second  (00 to 59)
      if (t5 < 0 or t5 > 59):
        return result
  except:
    return result  # error converting string to integers
  else:

    #=======================================
    #==== Logical Check on Valid Date ======
    if (wnow == 'Y' or wnow == 'NOW'):
      today = datetime.datetime.today()
      if (dateTimeAsIso8601(wdatetuple) < today.strftime(CheckCurrentDate)):
        return result                                   # ===  Date in the Past (error)            ===
    elif (wnow == 'PAST'):
      today = datetime.datetime.today()
      if (dateTimeAsIso8601(wdatetuple) > today.strftime(CheckCurrentDate)):
        return result                                   # ===  Date in the Past (error)            ===
    ...
    #===========================================
    #====   CHECK ONLY Resent an ISO Date   ====
    if (wmode == 'CHECK-ONLY'  or wmode == 'CHECK'  or wmode == 'VALIDATE'):
      result = timeTupleToMysqlTimeStamp(wdatetuple)    #=== send an ISO date                       ===
    else:
      result = wdatetuple                               #=== Send Tuple for further date processing ===

  return result                                         # ==== return TupleDate                     ===
...
def getDateRange(key, timeTuple = None, current=None):
  ...
  #================================================================
  #=== Special case for LastNnn (Last days - common procedure) ====
  ...
  #================================================================
  #=== Special case for LastNnn (Last days - common procedure) ====
  if string.find(key,'ADD') >=0 :
    nbrDay = string.replace(key,'ADD','')         # === special case for LastNnn (Last Days) ===
    if CheckNumeric(nbrDay):
      addingDay = int(nbrDay)                        # === special case for LastNnn (Last Days) ===
      key = 'ADD'
  ...
  #=== Common DateRange proc for All LASTnnn (nnn= nr of days, LAST7, LAST30, ...) ===
    ...
    beginTime = time.mktime(timeTuple) + (addingDay*86400)
    beginTuple = time.localtime(beginTime)
    endTuple   = timeTuple
  elif key == 'NEXT WEEK':
...
def getDateRangeStamp(key, timeTuple = None, current=None):
  "calculate the date range based on a key. returns week range by default"

  beginTuple, endTuple = getDateRange(key, timeTuple = timeTuple, current=current)
  try:
